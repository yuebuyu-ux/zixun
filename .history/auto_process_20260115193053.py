...
【格式要求】
1. 提取 10-15 条最热门、最重要的新闻。
2. 每条新闻格式：'新闻标题｜两句话深度解读(约50-80字，两句完整的话)'
   - 标题要简练有力。
   - 解读要包含背景或影响，而不仅仅是重复标题，确保内容充实。
...
"""
    ...
    try:
        ...
        if 'news_items' in data and isinstance(data['news_items'], list):
            ...
            for item in data['news_items']:
                if isinstance(item, str):
                    parts = item.split('｜')
                    if len(parts) > 1:
                        normalized_items.append({"title": parts[0], "summary": parts[1]})
                    else:
                        normalized_items.append({"title": item, "summary": ""})
        ...
        try:
            import re
            news_items = re.findall(r'"([^"+｜[^"]+)"', result)
            if news_items:
                print(f"通过正则找回 {len(news_items)} 条新闻")
                return {
                    "news_items": news_items,
                    "quote": {"text": "解析部分失败，但已尽力挽回", "author": "AI"}
                }
        except:
            pass
        return None
