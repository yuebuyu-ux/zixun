...
3. 第二行是干练引导语，包含 emoji，并提醒“先上汇总图”，语气权威实用。
4. 新闻主体使用连续的三级标题格式：### 🔸 序号. 新闻标题。
5. 每条新闻标题下一行紧跟 1-2 个关键词标签（如 #春运 #财经） + 一句话核心解读，解读基于 summary_short，长度 24-32 字。
6. 不分板块，随机排列顺序，不要新增空行。
7. 结尾用分割线 --- ，下一行输出“今日互动：{payload["ending"]}”。
            ...
            try:
                ...
                if not article_content:
                    ...
                    for i, item in enumerate(data['news_items'], 1):
                        ...
                        summary_short = (item.get("summary_short") or "").strip()
                        if summary_short:
                            article_content += f"{summary_short}\n"
