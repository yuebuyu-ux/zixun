            ...
            if source['type'] == 'manual_captcha':
                ...
                if "tophub.today" in source['url']:
                    ...
                    try:
                        ...
                        # === 晚报按钮定位逻辑优化 (遍历尝试版) ===
                        print("正在定位并尝试点击【晚报】切换按钮...")
                        
                        # 获取页面上所有可能的“晚报”元素（按钮、链接、span等）
                        # 排除侧边栏的常见干扰项（如果有明确的class可以排除更好，这里先用位置启发）
                        candidates = page.locator("text=晚报")
                        count = await candidates.count()
                        print(f"页面上共找到 {count} 个包含“晚报”文本的元素。")
                        ...
                        for i in range(count):
                            try:
                                ...
                                # 简单的启发式过滤：如果元素在页面太靠左（侧边栏），可能不是我们要的
                                # 假设侧边栏宽度小于 250px
                                if box['x'] < 200: 
                                    print(f"跳过第 {i+1} 个元素 (x={box['x']})，疑似侧边栏链接。")
                                    continue

                                print(f"尝试点击第 {i+1} 个候选元素 (x={box['x']}, y={box['y']})...")
                                await btn.click(force=True, timeout=1000)
                                
                                # 等待并检查内容变化
                                deadline = asyncio.get_running_loop().time() + 3.0 # 短暂等待
                                while asyncio.get_running_loop().time() < deadline:
                                    await page.wait_for_timeout(500)
                                    content_current = await page.inner_text("body")
                                    
                                    # 检查变化
                                    if abs(len(content_current) - len(content_early)) > 100 or content_current[:200] != content_early[:200]:
                                        print(f"成功获取【晚报】内容！(通过第 {i+1} 个按钮)")
                                        print(f"晚报字数: {len(content_current)}")
                                        merged = f"=== 早报内容 ===\n{content_early}\n\n=== 晚报内容 ===\n{content_current}"
