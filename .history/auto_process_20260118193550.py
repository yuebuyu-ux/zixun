            ...
            if source['type'] == 'manual_captcha':
                ...
                if "tophub.today" in source['url']:
                    ...
                    try:
                        ...
                        # 策略1：最精准 - 查找紧邻“早报”的“晚报”按钮（对应图片中的切换器）
                        # 侧边栏通常只有“晚报”链接，而中间的内容区会有“早报/晚报”切换
                        try:
                            morning_el = page.locator("text=早报").first
                            if await morning_el.count() > 0:
                                # 尝试在早报按钮的父容器中查找晚报
                                # 假设它们是兄弟节点或在同一父节点下
                                potential = morning_el.locator("xpath=..").locator("text=晚报").first
                                if await potential.count() > 0:
                                    evening_btn = potential
                                    print(">> 已锁定：位于【早报】旁边的晚报按钮。")
                        except Exception:
                            pass

                        # 策略2：次选 - 查找“今日简报”区域内的晚报
                        if not evening_btn:
                            try:
                                # 尝试找到包含“今日简报”文本的元素，然后在其附近找晚报
                                header = page.locator("text=今日简报").first
                                if await header.count() > 0:
                                    potential = header.locator("xpath=..").locator("xpath=..").locator("text=晚报").first
                                    if await potential.count() > 0:
                                        evening_btn = potential
                                        print(">> 已锁定：位于【今日简报】区域内的晚报按钮。")
                            except Exception:
                                pass

                        # 策略3：兜底 - 原有逻辑（可能会误触侧边栏，作为最后尝试）
                        if not evening_btn:
                            print(">> 警告：未能通过精确特征定位，尝试查找页面上任意可见的‘晚报’按钮...")
                        ...
                        if evening_btn and await evening_btn.count() > 0 and await evening_btn.first.is_visible():
                            print("准备点击【晚报】按钮...")
                            # 强制点击，避免被遮挡
                            await evening_btn.first.click(force=True)
                            print("已点击【晚报】(force=True)，等待内容切换...")
                            ...
                            while asyncio.get_running_loop().time() < deadline:
                                ...
                                if abs(len(content_late) - len(content_early)) > 100 or content_late[:200] != content_early[:200]:
                                    print(f"成功获取【晚报】内容。晚报字数: {len(content_late)}")
                                    merged = f"=== 早报内容 ===\n{content_early}\n\n=== 晚报内容 ===\n{content_late}"
                                    print(f"合并后总字数: {len(merged)}")
                                    return merged

                            print("未检测到内容显著变化（可能自动点击失败）。")
                            print("【需要人工介入】请在浏览器中手动点击【晚报】（如果存在），确认内容更新。")
...
1.  提供一个标题字段 "page_title"：
    - 结构：账号名（可选） + 早间/昨日速览 + 1-2个高关注度关键词 + 强价值点（如“影响你的生活/关键信息汇总”）
...
3.  生成 1 个结尾文案字段 "ending"：
    ...
    - 点缀：可加1个引导类emoji（如⭐/🔔）
4.  提取 15 条新闻（不足则按实际数量输出），每条必须包含以下三个字段：
