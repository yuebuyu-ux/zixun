...
def update_data_json(new_data):
    ...
    try:
        ...
        current_data['page_title'] = new_data.get('page_title', "今日热点 · 每日简报")
        current_data['header_html'] = new_data.get('header_html', "每天<span>60s</span>读懂世界")
...
async def main():
    ...
    try:
        # 添加默认标题，防止影响旧逻辑
        if 'page_title' not in data_processed:
            data_processed['page_title'] = "今日热点 · 每日简报"
        if 'header_title' not in data_processed:
            data_processed['header_html'] = "每天<span>60s</span>读懂世界" # 使用 html 字段支持 span
