...
def _parse_play_count(text):
    s = (text or "").strip()
    if not s:
        return None
    if "次播放" not in s and "播放" not in s:
        return None
...
def parse_tophub_douyin_rank_items(text, max_items=50):
    lines = [line.strip() for line in (text or "").splitlines() if line.strip()]
    cleaned = []
    for line in lines:
        if line in {"", ""}:
    ...
    while i < len(lines) and len(items) < max_items:
        ...
        while j < len(lines) and not lines[j]:
            j += 1
        if j < len(lines) and "次播放" not in lines[j]:
            author = lines[j]
            j += 1
        while j < len(lines) and not lines[j]:
            j += 1
        if j < len(lines) and ("次播放" in lines[j] or "播放" in lines[j]):
            plays_text = lines[j]
            plays = _parse_play_count(plays_text)
            j += 1
...
def choose_best_topic_from_tophub(items, model):
    prompt = f"""
你是短视频选题编辑。下面是一份“抖音泛生活榜”的TopHub榜单条目（含标题/作者/播放量）。
请只根据输入内容，选出 1 个最适合做成微头条的热点，并给出抖音搜索用的 query（以及2个备用query）。

选题标准：
1) 更适合写观点/情绪/生活经验，不依赖看完整视频才能成立；
2) 讨论空间大、冲突点清晰，但避免高风险敏感；
3) 优先选择播放量高、标题信息密度高的；
4) query 要短、明确（优先去掉#话题标签，只保留核心名词/短语）。

输出必须是纯 JSON：
{{
  "chosen_rank": number,
  "chosen_title": "string",
  "chosen_author": "string",
  "chosen_plays": number,
  "search_query": "string",
  "alt_queries": ["string","string"],
  "reason": "string",
  "risk_notes": ["string"]
}}

输入：
{json.dumps({"items": items[:50]}, ensure_ascii=False)}
"""
    content = call_llm_chat(
        messages=[
            {"role": "system", "content": "你擅长热点选题与抖音搜索策略。"},
...
async def fetch_douyin_search_videos(keyword, max_items=3, manual=True):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if manual and need_manual:
                print("\n" + "=" * 50, flush=True)
                print("【需要人工介入】抖音可能触发验证/登录。", flush=True)
                print(f"已打开：{search_url}", flush=True)
                print("=" * 50 + "\n", flush=True)
                if sys.stdin and sys.stdin.isatty():
...
def analyze_hot_and_videos(payload, model):
    prompt = f"""
你是一位短视频运营专家 + 今日头条微头条写作教练。请基于我提供的“TopHub 抖音泛生活榜条目 + 抖音搜索结果卡片信息”，做可执行拆解。
...
目标：
1) 以输入 JSON 中的 chosen_topic 为核心选题，不允许另起炉灶换题。
2) 从抖音视频卡片里挑 1-2 条作为“素材入口”（如果没有也要说明原因并给出替代策略）。
3) 产出：关键词矩阵、情绪/争议点、二创建议、合规风险提示、以及文章写作计划（标题备选+五段式要点）。
...
async def cmd_hot(args):
    ...
    items = parse_tophub_douyin_rank_items(text, max_items=args.max_items)
    if items:
        for it in items[: args.max_items]:
            meta = []
            if it.get("author"):
                meta.append(it["author"])
            if it.get("plays_text"):
                meta.append(it["plays_text"])
...
async def cmd_run(args):
    ...
    print(f"抓取 TopHub：{args.tophub_url}")
    tophub_text = await fetch_tophub_hot(args.tophub_url)
    hot_items = parse_tophub_douyin_rank_items(tophub_text, max_items=args.max_hot_items)
    if not hot_items:
        hot_items = parse_tophub_hot_items(tophub_text, max_items=args.max_hot_items)
    if not hot_items:
        raise RuntimeError("TopHub 热榜解析失败")

    print("正在让 AI 从 TopHub 里选出 1 个最适合写的热点...")
    try:
        chosen = choose_best_topic_from_tophub(hot_items, model=model)
    except Exception:
        first = hot_items[0] if hot_items else {}
        chosen = {
            "chosen_rank": int(first.get("rank") or 1),
            "chosen_title": str(first.get("title") or "").strip(),
            "chosen_author": str(first.get("author") or "").strip(),
            "chosen_plays": first.get("plays") if isinstance(first.get("plays"), int) else None,
            "search_query": str(first.get("title") or "").strip(),
            "alt_queries": [],
            "reason": "LLM 选题失败，退回使用榜单第一条。",
            "risk_notes": [],
        }

    search_query = str((chosen or {}).get("search_query") or "").strip()
    alt_queries = (chosen or {}).get("alt_queries") or []
    if not isinstance(alt_queries, list):
        alt_queries = []
    queries = [q.strip() for q in ([search_query] + [str(x) for x in alt_queries]) if q and q.strip()]
    if not queries:
        chosen_title = str((chosen or {}).get("chosen_title") or "").strip()
        if chosen_title:
            queries = [chosen_title]
    if not queries:
        raise RuntimeError("未能生成抖音搜索关键词")
    ...
    for kw in queries:
        ...
        try:
            ...
            videos = []
        douyin_all[kw] = videos
        if videos and not used_query:
            used_query = kw
            used_videos = videos
            break
    ...
    payload = {
        ...
        "hot_items": hot_items,
        "chosen_topic": {
            "rank": (chosen or {}).get("chosen_rank"),
            "title": (chosen or {}).get("chosen_title"),
            "author": (chosen or {}).get("chosen_author"),
            "plays": (chosen or {}).get("chosen_plays"),
            "search_query": search_query,
            "alt_queries": alt_queries,
            "reason": (chosen or {}).get("reason"),
            "risk_notes": (chosen or {}).get("risk_notes"),
        },
        "douyin_search": {
            "queries": queries,
            "used_query": used_query,
            "videos": used_videos,
            "all_results": douyin_all,
        },
        "constraints": {"audience": "30-55岁三四线城市用户为主"},
        "params": {
            "max_hot_items": args.max_hot_items,
            "videos_per_keyword": args.videos_per_keyword,
            "min_len": args.min_len,
            "max_len": args.max_len,
            "no_manual_douyin": args.no_manual_douyin,
        },
    }
...
def build_parser():
    ...
    p_run.add_argument("--json-path", default="")
