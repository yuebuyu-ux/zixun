...
3. 第二行是干练引导语，包含 emoji，并提醒“先上汇总图”，语气权威实用，用 Markdown 引用并加标识符，格式：> 引导语：内容。
4. 新闻标题使用二级标题格式：## 🔸 序号. 新闻标题。
5. 每条新闻标题下一行输出 1 个关键词标签（如 #财经），再下一行使用三级标题输出正文内容，正文基于 summary_long，可适度润色但不添加新事实。
